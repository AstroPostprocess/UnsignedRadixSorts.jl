...
Return the active CUDA key source and destination buffers for one radix pass.
...
- `ws`: CUDA OneSweep workspace containing the ping-pong destination buffer `ws.dst`.
