...
Return the active Metal key and permutation source/destination buffers for one
radix pass.
...
- `ws`: Metal OneSweep workspace containing key and permutation ping-pong buffers.
...
for KeyT in (UInt8, UInt16, UInt32, UInt64)
    @eval begin
        @inline function _select_pass_key_value_buffers(codes :: KeyV, ws :: OnesweepWorkspace{$KeyT, KeyV, OffsetV}, :: Val{Pass}) where {KeyV <: MtlDeviceVector{$KeyT}, OffsetV <: MtlDeviceVector{UInt32}, Pass}
            if isodd(Pass)
