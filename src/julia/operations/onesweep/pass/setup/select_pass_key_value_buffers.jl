...
Return the active key and permutation source/destination buffers for one radix
pass.
...
both ping-pong pairs. This function only returns buffer aliases and does not
...
- `ws`: Workspace containing key and permutation ping-pong buffers.
...
for KeyT in (UInt8, UInt16, UInt32, UInt64, UInt128)
    @eval begin
        @inline function _select_pass_key_value_buffers(codes :: KeyV, ws :: OnesweepWorkspace{$KeyT, KeyV, OffsetV}, :: Val{Pass}) where {KeyV <: Vector{$KeyT}, OffsetV <: Vector{UInt32}, Pass}
            if isodd(Pass)
